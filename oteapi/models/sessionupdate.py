...
from pydantic import ValidationError, root_validator, validator, Field
...
class OTESession(AttrDict):
    ...
    session_type: Optional[str] = Field(
        None,
        description="Describe the current session type"

    )
    session_id : Optional[str] = Field(
        None,
        description="Optional ID for referring to session data"
    )

    @root_validator(pre=True)
    def check_session_assignment(cls, values):
        """ check that session_type is defined if session_id is given """
        session_id, session_type = values.get('session_id'), values.get('session_type')
        if session_id is not None and session_type is None:
            raise ValueError('session_id cannot be defined without specifying session_type')
        return values
