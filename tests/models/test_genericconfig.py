...
def test_attribute_set(generic_config: CustomConfig) -> None:
    ...
    NOTE: Start from Pydantic v2, automatic type coercion for all types is not
    supported. For instance, `float` or `int` to `str`.
    One should instead use `typing.Annotated` to create custom types.
    ...
    UPDATE: In v2.4, a new configuration was introduced to restore this behavior:
...
def test_attribute_del_item(generic_config: CustomConfig) -> None:
    ...
    # Therefore, when deleted:
    # - It should no longer exist in the model's extras.
    # - It should also be absent from the model instance itself.
    # - It should also be removed from the set of user-set fields.
    assert "float" in generic_config.configuration
    ...
    # schema.
    # Therefore, when deleted:
    # - It should still exist in the model's schema.
    # - It should be removed from the model instance itself.
    # - It should also be removed from the set of user-set fields.
...
def test_attribute_reset_field(generic_config: CustomConfig) -> None:
    ...
    # "float" is not defined as a pydantic model field, i.e., it's not part of the
    # model schema, but rather the model extras.
    # Resetting it should behave similarly to deleting it.
    ...
    # Therefore, when reset:
    # - It should still exist in the model's schema.
    # - It should be reset to its default value.
    # - It should be removed from the set of user-set fields.
